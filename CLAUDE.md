...
- **Frontend** (`frontend/`, Vite + React 19) — форма (react-hook-form + zod), SSE-прогресс по нодам, view плана, PDF download. **UI-тема Halyk:** primary green `#00B14F` — токены `halyk`/`halyk-dark`/`halyk-light` в `frontend/tailwind.config.js` (`accent` тоже зелёный, `muted`=`#eef1f4`, `ink`=`#111827`); меняйте цвета через токены, не хардкодьте hex по компонентам. Frontend — build-time static (nginx), исходники НЕ смонтированы как volume → после правок нужен `docker compose up -d --build frontend` (не `restart`).

**LLM-роли (OpenAI-only):**
- Primary: **`gpt-5.4-nano`** (env `OPENAI_MODEL`) — используется ВЕЗДЕ: `generate_plan`, `vision_identify`, `parse_edit_intent` (через `with_structured_output`), LLM-as-judge, HITL `explain_and_ask`, `finalize_and_export`. Single-model подход — проще и дешевле, чем Pro/Flash split.
- Secondary: **`gpt-5.4-nano`** (env `OPENAI_MODEL_B`) — используется ТОЛЬКО как второй arm в A/B-evals.
...
- **LLM провайдер:** ТОЛЬКО OpenAI. Primary — `gpt-5.4-nano` (env `OPENAI_MODEL`). Secondary `gpt-5.4-nano` — только для A/B-evals (`OPENAI_MODEL_B`).
...
- **`patch_plan` рендерит правку через LLM + faithfulness-guard:** при `changed>0` правка применяется к структурному `state.plan`, затем `_rich_rerender` прогоняет выживший план через LLM (промпты `EDIT_RERENDER_*`, тот же skill itinerary-formatter, `temperature=0.2`) — так сохраняются halal-маркеры/описания/источники/таблица бюджета. **Guard `_covers_place_names`:** слабая nano-модель на edit-пути иногда анонимизирует блоки (теряет имена/🟢-маркеры/OSM-ссылки, рендерит «Категория: ресторан»); поэтому если LLM-рендер пустой ИЛИ покрывает <60% имён мест из плана — берётся `_render_markdown`, а это теперь **детерминированный RICH-рендер** (имена, OSM-ссылки, dietary-маркеры через `_dietary_marker`, per-day-итоги, таблица бюджета из `cost`, секция «Источники»), НЕ bare. После правки пересчитывается `plan_cost_breakdown` через `estimate_plan_cost` (до рендера, чтобы fallback видел таблицу). При `changed==0` (ничего не совпало) markdown сохраняется, но сверху добавляется явная пометка «правка не нашла совпадений» (`_prepend_noop_notice`) — чтобы no-op не выглядел как «правка не работает». `action="add"` реализован (`_apply_add`): берёт неиспользованного кандидата нужной категории, с MCP-фоллбэком `find_places`. `action="replace"` = remove(`intent.target`) + add(`intent.detail`) — добавляет именно `detail` (замену) через `target_override`, иначе «вместо музея парк» добавит снова музей. Поэтому `patch_plan` теперь async (делает LLM + MCP вызовы).
...
- **OpenAI rate limits:** на платном tier nano-модели имеют 30k+ RPM, evals на 10 примерах укладываются без проблем. Exponential backoff (3 попытки, base 1s) встроен в `llm/client.py` — на случай TPM-пиков и 5xx.
- **OpenAI cost:** nano-модели дешёвые, но не бесплатные. Evals на 10 примерах × 2 метрики × 2 arms ≈ $0.5–2 в зависимости от длины планов.
...
- **Сессия персистится в localStorage (`tp_session_id`):** `App.tsx` на маунте СНАЧАЛА пытается восстановить сохранённую сессию через `getSessionState` (и при наличии плана/ревью/финализации показать его), и только при 404/отсутствии создаёт новую (`createSession` + сохранение id). Без этого reload или browser-«назад» ремаунтил SPA → `createSession()` → новая пустая сессия → план терялся. Сессии in-memory (`api/session_store.py`) — после рестарта backend пропадают, restore ловит 404 и создаёт новую. `startOver` тоже перезаписывает сохранённый id. (Edge: уход со страницы ВО ВРЕМЯ генерации, статус ещё `draft` без плана, восстановит форму — покрыты готовый план/ревью/финализация/бюджет-пауза.)
- **Ссылки в плане — `target=_blank`:** `PlanView` рендерит markdown-`<a>` через проп `components` react-markdown (`a({node, ...props}) => <a … target=_blank rel=noopener>`). Иначе клик по OSM/Wikivoyage-ссылке уводит вкладку с SPA → выгрузка React → «назад» = сброс сессии (см. выше). НЕ убирать. `node` обязательно деструктурировать, иначе react-markdown прокинет его как невалидный DOM-атрибут.
- **PDF — blob-скачивание, не `<a href target=_blank>`:** `downloadPdf` в `api/client.ts` делает `fetch → blob → a.download` (имя `trip-<id8>.pdf`); `PlanView` держит busy/error-стейт и показывает ошибку при 409 («план не готов») / 404 («сессия пропала»). Endpoint `/sessions/{id}/pdf` (`api/export.py`) кеширует `pdf_bytes` в сессии, регенерит при смене `plan_markdown`, отдаёт `Content-Disposition: attachment`. Старый `<a target=_blank>` открывал пустую вкладку и молча падал, если сессия была сброшена.
